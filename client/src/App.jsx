...
import { getDrive, uploadFile, createFolder, deleteFile, deleteFolder, renameFile, renameFolder, toggleFile, toggleFolder, downloadFileUrl, extractFile, downloadZip } from './api';
import Sidebar from './components/Sidebar';
import FileGrid from './components/FileGrid';
import ContextMenu from './components/ContextMenu';
import FilePreview from './components/FilePreview';
import { Search, Plus, Upload, Loader2, Trash2, Edit2, Eye, Download, Star, Undo } from 'lucide-react';

function App() {
  const [currentFolder, setCurrentFolder] = useState(null);
  const [currentFilter, setCurrentFilter] = useState(null); // null (My Drive), 'recent', 'starred', 'trash'
  ...
  const [selectedItems, setSelectedItems] = useState(new Set()); // Set of IDs "type-id" e.g. "file-1", "folder-2"
  ...
  const handleFilterChange = (filter) => {
    ...
    setCurrentFolder(null); // Reset folder when changing filter (show root of filter)
    fetchData(null, filter);
  };

  const handleFolderClick = (folderId) => {
    if (currentFilter) {
      // If in a filter view (like Starred), we might want to just enter the folder normally
      // and exit the filter mode, OR drill down. 
      // Standard behavior: Exit filter mode and go to that folder.
  ...
  const handleUpload = async (e) => {
    ...
    // Upload only allowed in normal view
  ...
  const handleDelete = async (item = null, type = null) => {
    // Determine items to delete: specific item or selection
    let itemsToDelete = [];
    if (item) {
      ...
      selectedItems.forEach(key => {
        const [t, i] = key.split('-');
        itemsToDelete.push({ id: parseInt(i), type: t });
      });
    }
  ...
  const handleToggleTrash = async (item, type) => {
    try {
      const newStatus = !item.isTrashed; // Toggle trash status
      // Use standard delete if we want to move to trash, but we implemented soft-delete toggle
      // Actually our delete API deletes it. 
      // Let's us toggleFile API for Soft Delete (Trash) if we wanted reversibility.
      // But our Delete API implemented earlier does a hard/soft delete based on logic.
      // Let's just use the toggle API for "Move to Trash" / "Restore".
  ...
  const getContextMenuOptions = () => {
    ...
    if (type === 'file' || type === 'folder') {
      ...
      if (type === 'file') {
        options.push({ label: 'Download', icon: Download, onClick: () => window.location.href = downloadFileUrl(item.id) });
      }
    ...
    <div className="flex h-screen bg-transparent font-[Inter]" onClick={() => setContextMenu(null)}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
          ...
          <div className="flex items-center bg-white/5 backdrop-blur-xl rounded-2xl px-5 py-3 w-96 shadow-lg border border-white/10 focus-within:ring-1 ring-indigo-500/50 transition-all focus-within:bg-white/10 focus-within:shadow-indigo-500/20">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                // Debounce could be here, but for now simple callback
                // Or just assume user hits enter? Let's do live search with 500ms delay if we had lodash, but simple for now.
                // We'll update state and trigger fetch
        ...
        <div
          ...
          onClick={() => { /* clicking background clears selection */ if (selectedItems.size > 0) clearSelection(); }}
        >
          {/* Aurora Background */}
            ...
            <div className="flex gap-2">
              <button
                onClick={() => {
                  ...
                  selectedItems.forEach(key => {
                    const [t, i] = key.split('-');
                    items.push({ id: parseInt(i), type: t });
                  });
                  downloadZip(items);
                }}
